...
class Pow2(nn.Module):
    ...
    def forward(self, x):
        return torch.pow(x, 2).clamp(max=10, min=-10)
...
class Pow3(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.pow(x, 3).clamp(max=10, min=-10)
        return x
...
class Exp(nn.Module):
    ...
    def forward(self, x):
        return torch.exp(x).clamp(max=10, min=-10)
...
class Sinh(nn.Module):
    ...
    def forward(self, x):
        return torch.sinh(x).clamp(max=10, min=-10)
...
class Cosh(nn.Module):
    ...
    def forward(self, x):
        return torch.cosh(x).clamp(max=10, min=-10)
