...
def run_epoch(rank, worldsize, model, loader, loss_fn, optimizer, desc_default='', epoch=0, writer=None, verbose=1,
    ...
    for batch in logging_loader:  # logging loader might be a loader or a loader wrapped into tqdm
        ...
        if optimizer and (communicate_grad_every == 1 or just_communicated_grad) and (
                (steps % accum_iter == 0) or (steps == total_steps)):
            optimizer.zero_grad()

        preds = model(data)
        loss = loss_fn(preds, label) / accum_iter
        if optimizer:
            if communicate_grad and ((steps % accum_iter == 0) or (steps == total_steps)):
                loss.backward()
            elif (steps % accum_iter == 0) or (steps == total_steps):
                with model.no_sync():
                    loss.backward()
        ...
        if scheduler is not None and ((steps % accum_iter == 0) or (steps == total_steps)):
            scheduler.step(epoch - 1 + float(steps // 2) / (total_steps // 2))
...
def train_and_eval(rank, worldsize, tag, dataroot, test_ratio=0.0, cv_fold=0, reporter=None, metric='last',
    ...
    trainsampler, trainloader, validloader, testloader_, testtrainloader_, dataset_info = get_dataloaders(
        C.get()['dataset'], C.get()['batch'] // 2, dataroot, test_ratio, split_idx=cv_fold, distributed=worldsize > 1,
